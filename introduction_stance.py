...
def display():
    ...
    \n1: The targets are further divided into :red-background[fine-grained targets]. For instance, the target "migrants" is further divided into "refugees", "asylum seekers", "economic migrants", and "illegal migrants".
    ...
    st.write("""The targets we are interested in concern some highly specialized concepts in political science. Below we will demonstrate all targets and their fine-grained targets with their definitions and how they can be identified in the posts. And how the stances towards these targets can be determined.
                In addition, googling is allowed if you find some concepts difficult to understand.
    """)
    ...
    :red-background[This target can be identified through a direct mention or keywords like “illegal”, or inferred from the context,] e.g. people crossing the border without permission.
    """
    ex_refugee = """
    :blue[**Refugees**]: People who flee their home country due to war, prosecution and other harms. 
    :red-background[This target can be selected when the word "refugee" is mentioned. However, it can also be inferred from the context, e.g. people from Syria fleeing the war.]
    """
    ex_asylum_seekers = """
    ...
    :red-background[This target can be identified through direct mentioning of people seeking asylum]. 
    """
    ex_eco_migrants = """
    :blue[**Economic migrants**]: Those who leave their home country for better economic/job opportunities. In our dataset, economic migrants often involve, but not limited to :red-background[EU migrants working in the UK].
    """
    stance_migrants = """
    \n:green[**positive stance**]: The text expresses a welcoming attitude towards migrants, e.g. by emphasizing their :red-background[positive contribution (e.g. economic, cultural)] to the host country. Or showing :red-background[empathy/solidarity] towards their situation during the refugee crisis.
    \n:red[**negative stance**]: The text expresses a hostile attitude towards migrants, e.g. by emphasizing the negative impact (e.g. economic, cultural) of migrants on the host country. Or showing a :red-background[lack of empathy] towards their situation during the refugee crisis.
    """
    ...
    This target is usually identified through direct mention of :red-background["Turkey", "turkish government" or "Erdogan".]
    """
    ...
    This target is usually identified :red-background[through direct mention]. 
    """
    ex_refugee_quotas = """
    ...
    :red-background[This target can be identified through the word "quota" or the discussion about the distribution of the refugees among EU countries.]  Click this [link](https://www.bbc.com/news/world-europe-34329825) for more information.
    """
    ...
    \n:red[**negative stance**]: The text expresses a hostile attitude towards the migration policies, e.g. by emphasizing their :red-background[negative impact] on the host country or :red-background[negative role] in the refugee crisis.
    ...
    """
    stance_rp = """
    \n:green[**positive stance**]: Due to the negative nature of the events, it is :red-background[unlikely] that the post will have a positive stance towards these events.
    \n:red[**negative stance**]: The post either emphasizes the :red-background[suffering of the refugees] during these events (from pro-migrant perspective) or :red-background[criticizes that foreigners are coming to the country illegally through these events] (from anti-migrant perspective).
    """
    st.write("This includes phenomena occurring during refugees' journey, or the ways they land in the host country.")
    st.write("Hint: If the post doesn't talks abour refugees or asylum seekers, it is likely that the target is irrelevant.")
    st.write("**Fine-grained targets**")
    ...
    st.write("Hint: If the post doesn't talk about refugees or asylum seekers, it is likely that the target is irrelevant.")
    ex_refugee_camps = """
    :blue[**Refugee camps**]: Temporary shelters for refugees. This can be identified through direct mention or words like :red-background[“detention camps”.]
    """
    ex_refugee_status = """
    :blue[**Refugee status**]: The legal status of refugees, which might include the right to work in the receiving country. This can be identified through direct mention or words like :red-background[“asylum status”].
    """
    stance_reception = """
    \n:green[**positive stance**]: The text emphasizes the :red-background[necessity] of providing shelters and granting refugee status to the asylum seekers.
    \n:red[**negative stance**]: The text emphasizes the :red-background[poor conditions of the refugee camps or the lack of rights given to the refugees] (from pro-migrant perspective). Or it :red-background[opposes] the idea of hosting refugees, providing them with shelters or granting them asylum status. (from anti-migrant perspective).
    """
    ...
    """
    stance_ap = """
    \n:green[**positive stance**]: The text emphasizes the :red-background[necessity] of providing protection, legal rights, and compensation to the asylum seekers. Or praises the :red-background[effectiveness] of the asylum procedure.
    \n:red[**negative stance**]: The text emphasizes the :red-background[lack] of protection, legal rights, or compensation given to the asylum seekers, or the :red-background[inefficiency] of the asylum procedure. (from pro-migrant perspective). Or it :red-background[opposes the idea of providing protection, legal rights, or compensation to the asylum seekers.] Or that the current asylum procedure :red-background[facilitates the influx of refugees] into the country.
